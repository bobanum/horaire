...
export default class Horaire extends DOM {
	...
	static url_grille(nom) {
		return this.url_json('grille_' + nom + '.json');
	}
	static url_theme(nom) {
		return this.url_json('theme_' + nom + '.json');
	}
	/**
	...
	dom_form_themes() {
		...
		this.createElementIn(select, 'option', 'Standard', {
			value: 'standard',
		});
		this.createElementIn(select, 'option', 'Autre', {
			value: 'autre',
		});
	...
	static setEvents() {
		this.prototype.evt = {
			plage: {
				click: function () {
					var courant;
					if ((courant = document.querySelector('div.plage.courant'))) {
						courant.obj.deposer();
					} else {
